...
def test_streaming_fallback_to_non_streaming():
    ...
    def mock_call(messages, tools=None, callbacks=None, available_functions=None):
        ...
        crewai_event_bus.emit(llm, event=LLMStreamChunkEvent(chunk="Test chunk 1", response_id="test-response-id-1"))
        crewai_event_bus.emit(llm, event=LLMStreamChunkEvent(chunk="Test chunk 2", response_id="test-response-id-2"))
